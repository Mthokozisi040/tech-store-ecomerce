        ...
        {THEMES.map((themeOption) => (
          <button
            ...
            className={`w-full px-4 py-3 rounded-xl flex items-center gap-3 transition-colors hover:bg-base-300 ${
              themeOption.name === document.documentElement.getAttribute('data-theme')
          ...
          >
            ...
            <span className='text-base-content'>{themeOption.label}</span>
            ...
            <div className='ml-auto font-medium flex gap-1'>
              {themeOption.colours.map((colour, i) => (
                <span
                  key={i}
                  className={`inline-block w-4 h-4 rounded-full ml-1 ${colour}`}
                ></span>
              ))}
            </div>
          </button>
